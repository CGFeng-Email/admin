<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="contactList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="留言" align="center" prop="text" />
  ...
  methods: {
    ...
    getList() {
      ...
      if (null != this.daterangeCreateTime && '' != this.daterangeCreateTime) {
        ...
        var date = new Date();
        this.daterangeCreateTime[0] = date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate();
        this.daterangeCreateTime[1] = date.getFullYear()+"-"+(date.getMonth()+1)+"-"+(date.getDate()+1);
        this.queryParams.params["beginCreateTime"] = this.daterangeCreateTime[0];
        this.queryParams.params["endCreateTime"] = this.daterangeCreateTime[1];
      }
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除联系编号为"' + ids + '"的数据项？').then(function() {
